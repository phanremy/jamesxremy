...
class Item < ApplicationRecord
  ...
  validates :description, :uid, :price, :actual_quantity, :expected_quantity, presence: true
  ...
  scope :description_or_uid_query, lambda { |search|
    return if search.blank?

    where(arel_table[:description].matches("%#{I18n.transliterate(search)}%"))
      .or(where(arel_table[:uid].matches("%#{I18n.transliterate(search)}%")))
  }
